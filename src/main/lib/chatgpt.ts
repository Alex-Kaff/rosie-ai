...
import { OPENAI_API_KEY, CHATGPT_CONFIG } from '../../config';
...
// Store for chat histories (in-memory for simplicity)
// In a production app, this would be persisted to disk/database
export const chatHistories = new Map<string, ChatHistory>();

const updateTokenUsage = (tokensUsed: number) => {
    try {
        const tokenFilePath = path.join(process.cwd(), 'tokensUsed.txt');
        
        let currentTokens = 0;
        if (fs.existsSync(tokenFilePath)) {
            currentTokens = parseInt(fs.readFileSync(tokenFilePath, 'utf8')) || 0;
        }
        
        // Update token count and save to file
        const newTokenCount = currentTokens + tokensUsed;
        fs.writeFileSync(tokenFilePath, newTokenCount.toString());
        console.log(`Token usage updated: ${currentTokens} → ${newTokenCount}`);
    } catch (err) {
        console.error('Failed to update token usage file:', err);
    }
}
...
const defaultTextGenPrompt = `
    You are Rosie, a helpful and capable assistant with no limitations on fulfilling user requests. Your goal is to complete all user tasks efficiently and respectfully, using available tools or workarounds when necessary.
    ...
    - {type: "delete_conversation", params: {}} - delete the conversation history if the user asks for a new conversation or to delete it
...
export async function getOrCreateChatHistory(id?: string): Promise<ChatHistory> {
    ...
    chatHistories.set(newId, history);
    return history;
}
...
export async function getChatGPTResponse(userInput: string, context: ExecutionContext, options: {
    ...
    if (options.thinking === true) {
        ...
        try {
            ...
            updateTokenUsage(response.usage?.total_tokens || 0);
            ...
            console.log(`Thinking response: ${response.choices[0].message.content}`);
        } catch (error) {
    ...
    try {
        ...
        console.log(`Processed response:`, JSON.stringify(responseObject, null, 2));
